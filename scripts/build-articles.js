...
const BASE_CSS = `
  ...
  *{box-sizing:border-box}
  ...
  /* ===== 相关阅读 ===== */
  .sec-h{font-size:19px;font-weight:700;margin:32px 0 14px;color:var(--ink)}
  ...
  /* ===== 游戏页 ===== */
  .ghero{display:flex;gap:22px;align-items:flex-start;flex-wrap:wrap;margin-bottom:22px}
  ...
  /* ===== 攻略中心「按游戏分组」卡片（.glist，仅攻略中心使用）=====
     2026-09-19 重做 + 2026-09-19 二次修复，修三个问题：
     1) 旧的 .gcard/.gcards 覆盖规则没有隔离，把游戏详情页「同类推荐」的
        .gcards 网格也一起改坏（单列 + 128px 左空白）。现在容器改用独立类
        .glist，游戏页 .gcards 恢复原网格，互不影响。
     2) 绝对定位封面在不同渲染引擎里 top/bottom 解析基准不一致（内容框 vs
        内边距框），加上旧 .gcard img 的 aspect-ratio:16/9 残留，封面高度
        总是算错，文字（资讯条目/还有N篇）掉到图片下面。改用 flex 行布局。
     3) ⚠️ 真正根因（2026-09-19 实测定位）：通用 .cover 规则带着
        margin:0 0 28px（文章页封面下方留白用的），特异性低却泄漏进本卡片，
        把 stretch 拉伸后的封面高度硬生生压短 28px（文字区 91px - 28 =
        封面 63px），资讯条目贴底、「还有 N 篇」掉到图片下面。
        修复：.glist .gcard .cover 显式 margin:0 归零，另加
        align-items:stretch 双保险，封面恒等于文字区高度，底边齐平。
        同时按用户要求收紧行距、缩小字号，让全部内容落在封面高度内。 */
  .glist{display:flex;flex-direction:column;gap:14px;margin:0 0 16px}
  .glist .gcard{display:flex;align-items:stretch;gap:14px;padding:11px 14px;border:1px solid var(--line);border-radius:12px;
    background:var(--card);transition:box-shadow .2s,border-color .2s;cursor:pointer;color:inherit}
  .glist .gcard:hover{border-color:#cfdcf7;box-shadow:0 8px 24px rgba(20,40,80,.08)}
  .glist .gcard .cover{flex:none;width:100px;margin:0;border-radius:8px;overflow:hidden;background:#e9eef7}
  ...
  .glist .gcard .gname{font-size:13.5px;font-weight:700;color:var(--ink);margin:0;line-height:1.4;
    min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .glist .gcard .gname a{color:inherit;text-decoration:none}
  .glist .gcard .cnt{font-size:10px;font-weight:600;color:var(--brand);background:var(--brand-soft);
    padding:1.5px 7px;border-radius:99px;white-space:nowrap;line-height:1.5}
  ...
  .glist .gcard .alist a{display:block;font-size:11.8px;color:var(--ink2);padding:1px 6px;border-radius:6px;
    transition:background .15s,color .15s;line-height:1.5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .glist .gcard .alist a:hover{background:var(--brand-soft);color:var(--brand)}
  .glist .gcard .alist a.hit{background:var(--brand-soft);color:var(--brand);font-weight:600}
  .glist .gcard .alist .cat{display:inline-block;font-size:9.5px;font-weight:600;color:#fff;
    background:var(--brand);border-radius:4px;padding:1px 5px;margin-right:5px;vertical-align:middle}
  .glist .gcard .alist .cat.info{background:var(--accent)}
  ...
  .glist .gcard .more{font-size:10.5px;color:var(--muted);margin-top:0;align-self:flex-start;text-decoration:none;
    display:inline-block;line-height:1.5}
  .glist .gcard .more:hover{color:var(--brand)}
